...
def create_fio_report(fio: str, work_date: datetime.datetime) -> dict[str, tuple]:
    ...
    surname = fio.split()[0]
    name = fio.split()[1]
    work_date_start = datetime.datetime(year=work_date.year, month=work_date.month,
                                        day=work_date.day, hour=0, minute=1)
    work_date_end = datetime.datetime(year=work_date.year, month=work_date.month,
...
def tst(request):
    ...
    today = datetime.datetime(year=2024, month=4, day=29)
    # today = datetime.datetime.now()
    for fio in doers:
        context = create_fio_report(fio['doers'], today)

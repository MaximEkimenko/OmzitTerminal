...
class DefectCard(LoginRequiredMixin, RoleMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        """
        В шаблоне для отображения карточки вместо записи в базе используется словарь - имя поля и его значение,
        чтобы можно было выводить данные в цикле, а не вписывать каждое поле руками.
        """
        context = super().get_context_data(**kwargs)
...
def show_file(request, path: str):
    ...
    try:
        ...
        alert_message = f"Ошибка при открытии файла '{path}'"
